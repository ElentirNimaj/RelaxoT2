...
def extract_brain():
    ...
    for niiImage in list_nii:
        command=["python", EXTRACT_SCRIPT, "-i" , niiImage, "-a" , ATLAS]
        run(command)
    return None
...
def convert_nrrd():
    ...
    list_nrrd = glob.glob(DATASET + "/*_masked.nrrd")
...
def concat_echoes():
    ...
    path_nii = glob.glob(DATASET + "/*_masked.nii.gz")
    path_nii = [' -i ' + nii for nii in path_nii]
    
    command = CONCATENATE_SCRIPT + "".join(path_nii) + " -o " + DATASET + "\\Concatenated.nii.gz" 
    run(command)
